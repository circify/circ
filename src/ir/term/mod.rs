...
pub(super) const TERM_CACHE_LIMIT: usize = 10000000; //65536;
...
pub struct ComputationSubgraphIter {
    // (cs stacked, term)
    stack: Vec<(bool, Term)>,
    visited: TermSet,
    cs: ComputationSubgraph,
    idx: usize,
}

impl ComputationSubgraphIter {
    /// Make an iterator over the descendents of `root`.
    pub fn new(cs: &ComputationSubgraph) -> Self {
        Self {
            stack: cs.outs.iter().map(|c| (false, c.clone())).collect(),
            visited: TermSet::new(),
            cs: cs.clone(),
            idx: 0,
        }
...
impl std::iter::Iterator for ComputationSubgraphIter {
    type Item = (usize, Term);
    fn next(&mut self) -> Option<(usize, Term)> {
        self.idx += 1;
        while let Some((children_pushed, t)) = self.stack.last() {
            if self.visited.contains(t) {
                self.stack.pop();
                ...
                if let Op::Select = t.op{
                    if let Op::Const(Value::BitVector(_)) = &t.cs[1].op {
                        if self.cs.nodes.contains(&t.cs[1]){
                            self.stack.last_mut().unwrap().0 = true;
                            let last = self.stack.last().unwrap().1.clone();
                    ...
                    if let Op::Const(Value::BitVector(_)) = &t.cs[1].op {
                        self.stack.last_mut().unwrap().0 = true;
                        let last = self.stack.last().unwrap().1.clone(); 
                        if self.cs.nodes.contains(&last.cs[0]){
                            self.stack.push((false, last.cs[0].clone()));
                        }
                        if self.cs.nodes.contains(&last.cs[2]){
