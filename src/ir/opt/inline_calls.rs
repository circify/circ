...
fn inline(name: &str, params: BTreeMap<String, Term>, cs: &Computations) -> Vec<Term> {
    let mut res: Vec<Term> = Vec::new();
    let comp = cs.comps.get(name).unwrap();
