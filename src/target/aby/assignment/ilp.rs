...
pub fn assign_mut(c: &ComputationSubgraph, cm: &str, co: &ComputationSubgraph) -> SharingMap {
    ...
    let mut trunc_smap = TermMap::default();
...
pub fn calculate_cost_smart_dug(smap: &SharingMap, cm: &str, dug: &DefUsesGraph) -> f64 {
    ...
    for (t, to_ty) in smap {
        match &t.op() {
            ...
            Op::Select | Op::Store => {
                if let Sort::Array(_, _, length) = check(&t.cs()[0]){
                    cost = cost + (length as f64) * costs.ops.get(&t.op().to_string()).unwrap().get(to_ty).unwrap();
                    for arg_t in dug.def_uses.get(t).unwrap().iter() {
                        if smap.contains_key(&arg_t) {
                            let from_ty = smap.get(&arg_t).unwrap();
                            if from_ty != to_ty {
            ...
            _ => {
                // println!("op: {}", t.op);
                cost = cost + costs.ops.get(&t.op().to_string()).unwrap().get(to_ty).unwrap();
                for arg_t in dug.def_uses.get(t).unwrap().iter() {
                    if smap.contains_key(&arg_t) {
                        let from_ty = smap.get(&arg_t).unwrap();
                        if from_ty != to_ty {
