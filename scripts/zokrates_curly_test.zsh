...
function r1cs_test_count {
    zpath=$1
    threshold=$2
    o=$($BIN $zpath --language zsharp-curly r1cs --action count)
...
function pf_test {
    ...
    do
        ...
        $BIN examples/ZoKratesCurly/pf/$ex_name.zok --language zsharp-curly r1cs --action setup --proof-impl $proof_impl 
        # create a proof using the prover input (x,w) stored in the .pin file
        $ZK_BIN --inputs examples/ZoKratesCurly/pf/$ex_name.zok.pin --action prove --proof-impl $proof_impl
        # verify a proof using the verifier input (x) stored in the .vin file
        $ZK_BIN --inputs examples/ZoKratesCurly/pf/$ex_name.zok.vin --action verify --proof-impl $proof_impl
        # clean up
        rm -rf P V pi
    done
}
...
function pf_test_isolate {
    ...
    do
        ex_name=$1
        $BIN --zsharp-isolate-asserts true examples/ZoKratesCurly/pf/$ex_name.zok --language zsharp-curly r1cs --action setup --proof-impl $proof_impl
        $ZK_BIN --inputs examples/ZoKratesCurly/pf/$ex_name.zok.pin --action prove --proof-impl $proof_impl
        $ZK_BIN --inputs examples/ZoKratesCurly/pf/$ex_name.zok.vin --action verify --proof-impl $proof_impl
        rm -rf P V pi
    done
